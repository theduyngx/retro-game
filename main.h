 ...
 * 1. Message loop: feeds message into windows, and windows will process and send these
 *    messages to the application constantly to check for user input
 * 2. Window procedure (callback): when window receives a message, the message will be dispatched
 *    back into the window function, and this is where we will handle it with our own set of
 *    logic and instructions (aka coding).
 ...
 *    The difference between these 2 are unicode is broader, though it will go with a different
 *    set of functions for message handling (whereas ASCII can be handled with strcpy and whatnot).
 *
 * Programming in windows: EVERYTHING is a window.
 * Each 'area' on the screen that may mean something is considered a window. The window itself is,
 * of course, a window. So one of the first steps that we must do is creating a window class.
 *
 * ----------------
 *
 * Coding tips:
 ...
 *      What this means is C will look into the size of t, and initialize 0s accordingly. Say, t has
 *      a size of 16 bytes, then it will initialize 16 zeroes into the memory heap.
 *
 * - label:
 *      We can label checkpoints anywhere in code, and simply type "goto" whenever we need the code
 *      to jump to that label.
 *
 * - Input/Output parameter specifier
 *      Remember declarative programming languages? It is quite good to have input and output specs
 *      as they give more insights to the functionality to the compiler and hence compilers can give
 *      more intelligent warnings. Here's how we do it in C:
 ...
 * - Sleep(0):
 *      What Sleep(0) does is that it allows other threads to join it in the CPU instead of hogging the
 *      entire CPU to itself. However, the downside is that when there's no other thread that wants to
 *      take turn, the program (however small) will still take up the entire CPU anyway. Not to mention
 *      this thread work exchange lacks predictability and control.
 * - Memory allocation:
...
#define RES_HEIGHT 216                  // NES height resolution
